...
describe('Task API:', function () {
  ...
  describe('GET /api/tasks', function () {
    ...
    beforeEach(function (done) {
      request(app).get('/api/tasks').expect(200).expect('Content-Type', /json/).end(function (err, res) {
        if (err) {
          return done(err);
        }
        tasks = res.body;
  ...
  describe('GET /api/tasks/:id', function () {
    ...
    beforeEach(function (done) {
      request(app).get('/api/tasks/' + newTask.id).expect(200).expect('Content-Type', /json/).end(function (err, res) {
        if (err) {
          return done(err);
        }
        task = res.body;
  ...
  describe('PUT /api/tasks/:id', function () {
    ...
    beforeEach(function (done) {
      request(app).put('/api/tasks/' + newTask.id).send({
        name: 'Updated Task',
  ...
  describe('DELETE /api/tasks/:id', function () {
    ...
    it('should respond with 204 on successful removal', function (done) {
      request(app)['delete']('/api/tasks/' + newTask.id).expect(204).end(function (err, res) {
        if (err) {
          return done(err);
        }
        done();
    ...
    it('should respond with 404 when task does not exist', function (done) {
      request(app)['delete']('/api/tasks/' + newTask.id).expect(404).end(function (err, res) {
        if (err) {
          return done(err);
        }
        done();
...
//# sourceMappingURL=task.integration.js.map
